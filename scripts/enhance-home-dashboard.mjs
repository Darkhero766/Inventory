...
const home = String.raw`function HomePage(){
  ...
  return <div className="mobile-page home-dashboard fade-up mx-auto">
    ...
    <section className="home-dashboard__hero mb-4 overflow-hidden rounded-[26px] p-5 text-white sm:p-6">
      ...
      <div className="relative">
        <div className="flex items-center justify-between gap-3">
          <div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-white/55">Business overview</p><p className="mt-1 text-xs text-white/65">All completed sales</p></div>
          <Link href="/sales" className="home-dashboard__hero-link">Open sales <ArrowRight className="h-3.5 w-3.5"/></Link>
        </div>
        <div className="mt-5 flex items-end justify-between gap-4">
          <div><p className="text-[36px] font-extrabold leading-none tracking-[-.065em]">{compact(totalSales)}</p><p className="mt-2 text-[11px] text-white/55">{sales.length} completed sale{sales.length===1?'':'s'}</p></div>
          <div className="home-dashboard__today-chip"><span className="block text-[9px] uppercase tracking-[.14em] text-white/50">Today</span><strong className="mt-0.5 block text-sm">{compact(todayRevenue)}</strong><span className="mt-0.5 block text-[9px] text-white/45">{todaySales.length} sale{todaySales.length===1?'':'s'}</span></div>
        </div>
        <div className="mt-5 grid grid-cols-2 gap-2.5"><div className="home-dashboard__hero-stat"><span>Profit today</span><strong>{compact(todayProfit)}</strong></div><div className="home-dashboard__hero-stat"><span>Inventory units</span><strong>{products.reduce((s,p)=>s+p.quantity,0).toLocaleString('en-IN')}</strong></div></div>
      </div>
    </section>
    <section className="mb-5"><div className="mb-2.5 flex items-end justify-between"><div><p className={pageKicker}>Key numbers</p><h2 className="mt-1 text-[18px] font-extrabold tracking-[-.045em]">Store health</h2></div></div>
    ...
    <section className="mb-5"><div className="mb-2.5 flex items-end justify-between"><div><p className={pageKicker}>Shortcuts</p><h2 className="mt-1 text-[18px] font-extrabold tracking-[-.045em]">Quick actions</h2></div></div>
      <div className="grid grid-cols-3 gap-2.5"><Link href="/sales" className="home-dashboard__action"><span className="home-dashboard__action-icon"><ShoppingCart className="h-4 w-4"/></span><strong>New sale</strong><small>Checkout</small></Link><Link href="/inventory" className="home-dashboard__action"><span className="home-dashboard__action-icon"><PackagePlus className="h-4 w-4"/></span><strong>Inventory</strong><small>Manage stock</small></Link><Link href="/customers/new" className="home-dashboard__action"><span className="home-dashboard__action-icon"><Users className="h-4 w-4"/></span><strong>Customer</strong><small>Add customer</small></Link></div>
    </section>
    {(low.length>0||out.length>0||overdue>0)&&<section className="mb-5"><div className="home-dashboard__attention rounded-[22px] p-4"><div className="flex items-start gap-3"><span className="home-dashboard__attention-icon"><CircleAlert className="h-4 w-4"/></span><div className="min-w-0 flex-1"><p className="text-sm font-extrabold">Needs attention</p><p className="mt-1 text-[11px] leading-relaxed text-[hsl(var(--muted-foreground))]">{[out.length>0?out.length+' out of stock':null,low.length>0?low.length+' low stock':null,overdue>0?overdue+' overdue EMI':null].filter(Boolean).join(' · ')}</p></div><button onClick={()=>setLocation(overdue>0?'/customers?filter=overdue':'/inventory')} className="home-dashboard__attention-link">Review</button></div></div></section>}
    <section><div className="mb-2.5 flex items-end justify-between"><div><p className={pageKicker}>Shelf activity</p><h2 className="mt-1 text-[18px] font-extrabold tracking-[-.045em]">Recent products</h2></div><Link href="/inventory" className="text-[11px] font-bold">View all</Link></div>
      <div className="space-y-2">{recent.map(product=><Link href={'/product/'+product.id} key={product.id} className="home-dashboard__product"><ProductImage product={product} className="h-[54px] w-[54px] shrink-0 rounded-[14px]"/><span className="min-w-0 flex-1"><strong className="block truncate text-[13px]">{product.name}</strong><span className="mt-1 block truncate text-[10px] text-[hsl(var(--muted-foreground))]">{product.brand} · {product.sku}</span><span className="mt-1.5 block text-[10px] font-semibold">{product.quantity} in stock</span></span><div className="flex shrink-0 items-center gap-2"><StatusPill product={product}/><ArrowRight className="h-3.5 w-3.5 text-[hsl(var(--muted-foreground))]"/></div></Link>)}</div>
    </section>
  </div>;
}`;
