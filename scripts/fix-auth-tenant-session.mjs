...
const replacements = [
  ...
  [
    ...
    "const loaded=await hydrateInventoryState();if(!alive)return;write(SESSION_KEY,s);write(PROFILE_KEY,s);setSession(s);if(!loaded)console.warn('[auth] inventory hydration failed for authenticated user',user.id);",
  ],
  [
