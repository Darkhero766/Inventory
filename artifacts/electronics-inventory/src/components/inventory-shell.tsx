...
export function InventoryShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell flex min-h-[100dvh] text-[hsl(var(--foreground))]">
    <aside className={`inventory-sidebar fixed inset-y-0 left-0 z-40 flex w-[272px] flex-col border-r border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar)/.96)] px-4 py-5 shadow-[18px_0_60px_rgba(15,23,42,.04)] backdrop-blur-xl transition-transform duration-300 md:relative md:translate-x-0 md:shadow-none ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex items-center justify-between px-2"><Link href="/" onClick={close} className="flex min-w-0 items-center gap-3"><span className="brand-mark flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] bg-[hsl(var(--sidebar-primary))] text-[hsl(var(--sidebar-primary-foreground))]"><Sparkles className="h-4 w-4" /></span><span className="min-w-0"><strong className="block truncate text-sm tracking-[-.02em]">Keystone</strong><span className="font-mono text-[9px] uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">Stock desk</span></span></Link><button className="rounded-xl p-2 md:hidden" onClick={close} aria-label="Close menu"><X className="h-5 w-5" /></button></div>
      <div className="mt-9"><p className="mb-3 px-3 font-mono text-[9px] font-bold uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">Workspace</p>{nav.map(({href,label,icon:Icon})=><Link key={href} href={href} onClick={close} className={`mb-1 flex items-center gap-3 rounded-[14px] px-3.5 py-3 text-sm transition-all ${active(href)?'bg-[hsl(var(--sidebar-accent))] font-bold text-[hsl(var(--sidebar-accent-foreground))]':'text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--foreground))]'}`}><Icon className="h-[17px] w-[17px]"/><span>{label}</span></Link>)}</div>
      <div className="mt-8"><p className="mb-3 px-3 font-mono text-[9px] font-bold uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">Manage</p><Link href="/stock-history" onClick={close} className={`mb-1 flex items-center gap-3 rounded-[14px] px-3.5 py-3 text-sm transition-all ${active('/stock-history')?'bg-[hsl(var(--sidebar-accent))] font-bold text-[hsl(var(--sidebar-accent-foreground))]':'text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]'}`}><BarChart3 className="h-[17px] w-[17px]"/>Stock history</Link></div>
