...
export async function cloudCreateSale(sale: any, purchaseCost: number, profit: number) {
  ...
  if (saleInsert.error) throw new Error(`Sale could not be saved: ${saleInsert.error.message}`);
...
export async function cloudCreateEmiPlan(plan: any, payments: any[], dbSaleId?: string) {
  ...
  let emiInsert = await supabase.from('emi_plans').insert({ ...baseEmiRow, client_id }).select('id').single(); if (emiInsert.error && /client_id.*emi_plans|emi_plans.*client_id/i.test(emiInsert.error.message)) emiInsert = await supabase.from('emi_plans').insert(baseEmiRow).select('id').single(); if (emiInsert.error) throw new Error(`EMI plan could not be saved: ${emiInsert.error.message}`);
