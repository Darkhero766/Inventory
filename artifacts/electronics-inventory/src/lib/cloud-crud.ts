...
export async function cloudSaveProduct(product: any, existingId?: string) {
  ...
  const client_id = String(product.id || crypto.randomUUID());
...
async function resolveCustomerDbId(value: string | undefined, owner_id: string) {
  ...
  const byId = await supabase.from('customers').select('id').eq('id', value).eq('owner_id', owner_id).maybeSingle();
  if (byId.data?.id) return byId.data.id;
  const byClient = await supabase.from('customers').select('id').eq('client_id', value).eq('owner_id', owner_id).maybeSingle();
...
export async function cloudUpsertCustomer(customer: any, existingId?: string) {
  ...
  const owner_id = await ownerId();
  const client_id = String(customer.id || crypto.randomUUID());
...
export async function cloudCreateSale(sale: any, purchaseCost: number, profit: number) {
  ...
  const owner_id = await ownerId();
  const client_id = String(sale.id || crypto.randomUUID());
  const customer_id = await resolveCustomerDbId(sale.customerId, owner_id);
  const saleRow = { owner_id, client_id, invoice_number: sale.invoice ?? `INV-${Date.now()}`, customer_id, sale_date: sale.date ?? new Date().toISOString(), subtotal: Number(sale.subtotal ?? 0), discount_type: sale.discountType ?? null, discount_value: Number(sale.discountValue ?? 0), discount_amount: Number(sale.discount ?? 0), final_amount: Number(sale.total ?? 0), purchase_cost: purchaseCost, profit, payment_method: sale.payment ?? '', status: 'COMPLETED' };
  const { data, error: saleError } = await supabase.from('sales').insert(saleRow).select('id').single();
  if (saleError) throw new Error(`Sale could not be saved: ${saleError.message}`);
  const dbSaleId = data.id;
  const items = (sale.items ?? []).map((item: any) => ({ id: crypto.randomUUID(), owner_id, client_id: `${client_id}:${item.productId}`, sale_id: dbSaleId, product_id: item.productId, quantity: Number(item.quantity ?? 0), unit_price: Number(item.price ?? 0), discount: Number(item.discount ?? 0), final_price: Number(item.price ?? 0) * Number(item.quantity ?? 0) - Number(item.discount ?? 0) }));
  if (items.length) {
    const { error: itemError } = await supabase.from('sale_items').insert(items);
    if (itemError) {
      await supabase.from('sales').delete().eq('id', dbSaleId).eq('owner_id', owner_id);
      throw new Error(`Sale items could not be saved: ${itemError.message}`);
    }
  }
  return dbSaleId;
}
...
export async function cloudCreateEmiPlan(plan: any, payments: any[], dbSaleId?: string) {
  ...
  const owner_id = await ownerId();
  let sale_id = dbSaleId ?? null;
  if (!sale_id) {
    const saleRes = await supabase.from('sales').select('id').eq('client_id', plan.saleId).eq('owner_id', owner_id).maybeSingle();
    sale_id = saleRes.data?.id ?? null;
  }
  const customer_id = await resolveCustomerDbId(plan.customerId, owner_id);
  if (!sale_id) throw new Error('The sale record could not be found for the EMI plan.');
  if (!customer_id) throw new Error('The EMI customer could not be found.');
  const client_id = String(plan.id || crypto.randomUUID());
  const emiRow = { owner_id, client_id, sale_id, customer_id, total_amount: Number(plan.totalAmount ?? 0), down_payment: Number(plan.downPayment ?? 0), financed_amount: Number(plan.financedAmount ?? 0), emi_amount: Number(plan.emiAmount ?? 0), installments: Number(plan.installments ?? 1), paid_installments: 0, outstanding_amount: Number(plan.outstandingAmount ?? plan.financedAmount ?? 0), start_date: plan.startDate, next_due_date: plan.nextDueDate ? String(plan.nextDueDate).slice(0,10) : null, end_date: plan.endDate, frequency: plan.frequency ?? 'MONTHLY', status: plan.status ?? 'ACTIVE' };
  const { data, error } = await supabase.from('emi_plans').insert(emiRow).select('id').single();
  if (error) throw new Error(`EMI plan could not be saved: ${error.message}`);
  const rows = payments.map((p: any) => ({ id: crypto.randomUUID(), owner_id, client_id: `${client_id}:${p.installmentNumber}`, emi_plan_id: data.id, installment_number: Number(p.installmentNumber), due_date: String(p.dueDate).slice(0,10), amount: Number(p.amount ?? 0), paid_date: p.paidDate ?? null, status: p.status ?? 'UPCOMING' }));
  if (rows.length) {
    const { error: paymentError } = await supabase.from('emi_payments').insert(rows);
    if (paymentError) {
      await supabase.from('emi_plans').delete().eq('id', data.id).eq('owner_id', owner_id);
      throw new Error(`EMI schedule could not be saved: ${paymentError.message}`);
    }
  }
  return data.id;
}
