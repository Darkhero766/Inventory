...
export async function cloudSaveProduct(product: any, existingId?: string) {
  ...
  const row = {
    owner_id, name: product.name, brand: product.brand ?? '', category: product.category ?? '',
    model: product.model ?? '', sku: product.sku ?? '', serial_number: product.serialNumber ?? null,
    imei: product.imei ?? null, purchase_price: Number(product.purchasePrice ?? 0),
    selling_price: Number(product.sellingPrice ?? 0), stock: Number(product.quantity ?? 0),
    image_url: product.image ?? null, updated_at: new Date().toISOString(),
  };
  if (existingId) {
...
export async function cloudCreateSale(sale: any, purchaseCost: number, profit: number) {
  ...
  const { error: saleError } = await supabase.from('sales').insert({
    id, owner_id, customer_id: sale.customerId ?? null, invoice_number: sale.invoice ?? `INV-${Date.now()}`,
    sale_date: sale.date ?? new Date().toISOString(), subtotal: Number(sale.subtotal ?? 0),
    discount_type: sale.discountType ?? null, discount_value: Number(sale.discountValue ?? 0),
    discount_amount: Number(sale.discount ?? 0), final_amount: Number(sale.total ?? 0), purchase_cost: purchaseCost,
    profit, payment_method: sale.payment ?? '', status: 'COMPLETED',
  });
  if (saleError) throw saleError;
  const items = (sale.items ?? []).map((item: any) => ({
    id: crypto.randomUUID(), owner_id, sale_id: id, product_id: item.productId,
    quantity: Number(item.quantity ?? 0), unit_price: Number(item.price ?? 0), discount: Number(item.discount ?? 0),
    final_price: Number(item.price ?? 0) * Number(item.quantity ?? 0) - Number(item.discount ?? 0),
  }));
  ...
  for (const item of sale.items ?? []) await cloudAdjustStock(item.productId, -Number(item.quantity ?? 0));
  return id;
}
...
export async function cloudCreateEmiPlan(plan: any, payments: any[]) {
  ...
  const { error } = await supabase.from('emi_plans').insert({
    id: plan.id, owner_id, sale_id: plan.saleId, customer_id: plan.customerId,
    total_amount: plan.totalAmount, down_payment: plan.downPayment, financed_amount: plan.financedAmount,
    emi_amount: plan.emiAmount, installments: plan.installments, paid_installments: 0,
    outstanding_amount: plan.financedAmount, start_date: plan.startDate, next_due_date: plan.nextDueDate,
    end_date: plan.endDate, frequency: plan.frequency, status: 'ACTIVE',
  });
  if (error) throw error;
  ...
  if (rows.length) {
    ...
    if (paymentError) { await supabase.from('emi_plans').delete().eq('id', plan.id).eq('owner_id', owner_id); throw paymentError; }
  }
  return plan.id;
}
...
export async function cloudMarkEmiPaid(payment: any, plan: any) {
  ...
  const { error: planError } = await supabase.from('emi_plans').update({
    paid_installments: plan.paidInstallments, outstanding_amount: plan.outstandingAmount,
    next_due_date: plan.nextDueDate, status: plan.status,
  }).eq('id', plan.id).eq('owner_id', owner_id);
