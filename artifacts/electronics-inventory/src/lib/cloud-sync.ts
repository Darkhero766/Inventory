...
export function syncInventoryState(){const client=supabase;if(!client||typeof window==='undefined'||hydrating)return;if(timer)clearTimeout(timer);timer=setTimeout(async()=>{timer=undefined;if(!(await ensureSession()))return;const ownerId=await currentUserId();if(!ownerId)return;localStorage.setItem(OWNER_KEY,ownerId);const state=snapshot();const {error}=await client.rpc('sync_inventory_snapshot',{p_state:state});if(error)console.warn('[cloud] sync failed:',error.message);},350);}
