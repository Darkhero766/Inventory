...
async function hydrateRelational() {
  const client = supabase; if (!client || !(await ensureSession())) return false; const ownerId = await currentUserId(); if (!ownerId) return false;
  const [productsRes, customersRes, salesRes, itemsRes, emiRes, paymentsRes] = await Promise.all([
    ...
    // client_id is not needed by the UI; omitting it keeps hydration compatible with older sales tables.
  ...
  const results = [productsRes, customersRes, salesRes, itemsRes, emiRes, paymentsRes]; const failed = results.find(r => r.error);
  ...
  for (const row of itemsRes.data ?? []) { const list = itemsBySale.get(row.sale_id) ?? []; list.push({productId:(row as any).products?.id??row.product_id,productName:(row as any).products?.name??'Product',quantity:Number(row.quantity??0),price:Number(row.unit_price??0),discount:Number(row.discount??0),serialNumber:(row as any).products?.serial_number??undefined,imei:(row as any).products?.imei??undefined}); itemsBySale.set(row.sale_id,list); }
  const sales = rawSales.map((s:any) => ({id:s.id,date:s.sale_date,invoice:s.invoice_number,customerId:s.customer_id??undefined,customerName:s.customers?.name??undefined,items:(itemsBySale.get(s.id)??[]) as Array<Record<string,unknown>>,subtotal:Number(s.subtotal??0),discount:Number(s.discount_amount??0),total:Number(s.final_amount??0),payment:s.payment_method??'',purchaseCost:Number(s.purchase_cost??0),profit:Number(s.profit??0),discountType:s.discount_type??undefined,discountValue:Number(s.discount_value??0),status:s.status}));
  const emiPlans = rawEmi.map((e:any) => ({id:e.id,saleId:e.sale_id,customerId:e.customer_id,totalAmount:Number(e.total_amount??0),downPayment:Number(e.down_payment??0),financedAmount:Number(e.financed_amount??0),emiAmount:Number(e.emi_amount??0),installments:Number(e.installments??0),paidInstallments:Number(e.paid_installments??0),outstandingAmount:Number(e.outstanding_amount??0),startDate:e.start_date,nextDueDate:e.next_due_date,endDate:e.end_date,frequency:e.frequency,status:e.status}));
  const emiPayments = (paymentsRes.data??[]).map((p:any) => ({id:p.id,emiPlanId:p.emi_plan_id,installmentNumber:Number(p.installment_number??0),dueDate:p.due_date,amount:Number(p.amount??0),paidDate:p.paid_date??undefined,status:p.status}));
