...
async function hydrateRelational() {
  ...
  const [productsRes, customersRes, salesRes, itemsRes, emiRes, paymentsRes] = await Promise.all([
    ...
    client.from('emi_plans').select('id,owner_id,sale_id,customer_id,total_amount,down_payment,financed_amount,emi_amount,installments,paid_installments,outstanding_amount,start_date,next_due_date,end_date,frequency,status,interest_rate,total_interest').eq('owner_id', ownerId),
