...
async function hydrateRelational() {
  ...
  for (const row of itemsRes.data ?? []) {
    ...
    list.push({ productId:(row as any).products?.id??row.product_id, productName:(row as any).products?.name??'Product', quantity:Number(row.quantity??0), price:Number(row.unit_price??0), discount:Number(row.discount??0), serialNumber:(row as any).products?.serial_number??undefined, imei:(row as any).products?.imei??undefined });
