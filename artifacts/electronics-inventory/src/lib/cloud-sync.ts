...
export async function hydrateInventoryState() {
  ...
  bootPromise = (async () => {
    ...
    try {
      ...
      const ownerId = await currentUserId();
      if (!ownerId) return false;
      const { data, error } = await supabase.from('inventory_state').select('state').eq('owner_id', ownerId).eq('workspace_key','default').maybeSingle();
...
export function syncInventoryState() {
  ...
  timer=setTimeout(async()=>{
    ...
    if (relationalError) console.warn('[cloud] snapshot sync failed:',relationalError.message);
    const { error } = await supabase.from('inventory_state').upsert({owner_id:ownerId,workspace_key:'default',state,updated_at:new Date().toISOString()},{onConflict:'owner_id,workspace_key'});
