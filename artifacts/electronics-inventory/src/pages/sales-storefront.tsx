...
export default function SalesStorefront() {
  ...
  return <div className="mx-auto max-w-6xl pb-36 fade-up">
    ...
    <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
      {categories.map(c => <button key={c} onClick={() => setCategory(c)} className={`shrink-0 rounded-full px-4 py-2 text-xs font-bold transition ${category === c ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))]'}`}>{c}</button>)}
    </div>
      ...
      {filtered.map(product => {
        ...
        return <article key={product.id} className="flex min-h-[118px] items-center gap-3 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
          ...
          <div className="min-w-0 flex-1 self-stretch py-1">
            <p className="truncate text-[10px] font-bold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">{product.brand}</p>
            <h2 className="mt-1 truncate text-sm font-extrabold">{product.name}</h2>
            <p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{money(product.sellingPrice)} · {product.quantity} available</p>
            <div className="mt-2 flex items-center gap-2">
              {inCart > 0 && <div className="flex items-center rounded-xl bg-[hsl(var(--muted))] p-0.5"><button onClick={() => change(product.id, -1)} className="rounded-lg p-1"><Minus className="h-3 w-3"/></button><span className="w-5 text-center text-xs font-bold">{inCart}</span><button onClick={() => change(product.id, 1)} className="rounded-lg p-1"><Plus className="h-3 w-3"/></button></div>}
              <button onClick={() => add(product)} disabled={inCart >= product.quantity} className="inline-flex items-center gap-1.5 rounded-xl bg-[hsl(var(--primary))] px-3 py-1.5 text-[11px] font-bold text-[hsl(var(--primary-foreground))] disabled:opacity-50"><Plus className="h-3.5 w-3.5"/>{inCart ? 'Add more' : 'Add'}</button>
            </div>
          </div>
        </article>;
      })}
    ...
    {count > 0 && <div className="fixed inset-x-3 bottom-20 z-40 mx-auto max-w-3xl sm:bottom-5">
