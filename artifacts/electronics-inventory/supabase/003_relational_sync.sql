...
-- Reconcile the browser's durable state into the relational model in one transaction.
-- This is intentionally a SECURITY INVOKER function: RLS and the authenticated JWT still apply.
...
begin
  ...
  -- Products first because sale items reference them.
  for item in select * from jsonb_array_elements(coalesce(p_state->'keystone-products','[]'::jsonb)) loop
    ...
    values(
      item->>'id', coalesce(item->>'name','Unnamed'), coalesce(item->>'brand','Unknown'), coalesce(item->>'category','Accessories'),
      coalesce(item->>'model',''), coalesce(item->>'sku',item->>'id'), nullif(item->>'serialNumber',''), nullif(item->>'imei',''),
      coalesce((item->>'purchasePrice')::numeric,0), coalesce((item->>'sellingPrice')::numeric,0), coalesce((item->>'mrp')::numeric,0),
      greatest(0,coalesce((item->>'quantity')::integer,0)), greatest(0,coalesce((item->>'minStock')::integer,0)), coalesce(item->>'warranty',''), nullif(item->>'image','')
    )
    on conflict (client_id) do update set
      name=excluded.name,brand=excluded.brand,category=excluded.category,model=excluded.model,sku=excluded.sku,
      serial_number=excluded.serial_number,imei=excluded.imei,purchase_price=excluded.purchase_price,selling_price=excluded.selling_price,
      mrp=excluded.mrp,stock=excluded.stock,min_stock=excluded.min_stock,warranty=excluded.warranty,image_url=excluded.image_url;
  ...
  -- Sales and their line items are reconciled after customers/products exist.
  for item in select * from jsonb_array_elements(coalesce(p_state->'keystone-sales','[]'::jsonb)) loop
    customer_id := null;
    if nullif(item->>'customerId','') is not null then select id into customer_id from customers where client_id=item->>'customerId' limit 1; end if;
    ...
    for v in select * from jsonb_array_elements(coalesce(item->'items','[]'::jsonb)) loop
      select id into product_id from products where client_id=v->>'productId' limit 1;
      if product_id is null then continue; end if;
      insert into sale_items(client_id,sale_id,product_id,quantity,unit_price,discount,final_price)
      values((item->>'id')||':'||coalesce(v->>'productId','item'),sale_id,product_id,greatest(1,coalesce((v->>'quantity')::integer,1)),coalesce((v->>'price')::numeric,0),coalesce((v->>'discount')::numeric,0),coalesce((v->>'price')::numeric,0))
  ...
  for item in select * from jsonb_array_elements(coalesce(p_state->'keystone-emi-plans','[]'::jsonb)) loop
    customer_id := null; sale_id := null;
    select id into customer_id from customers where client_id=item->>'customerId' limit 1;
    select id into sale_id from sales where client_id=item->>'saleId' limit 1;
    if customer_id is null or sale_id is null then continue; end if;
    ...
    returning id into emi_id;
    synced_emi := synced_emi + 1;
  end loop;

  for item in select * from jsonb_array_elements(coalesce(p_state->'keystone-emi-payments','[]'::jsonb)) loop
    select id into emi_id from emi_plans where client_id=item->>'emiPlanId' limit 1;
    if emi_id is null then continue; end if;
    insert into emi_payments(client_id,emi_plan_id,installment_number,due_date,amount,paid_date,status)
    values(item->>'id',emi_id,greatest(1,coalesce((item->>'installmentNumber')::integer,1)),(item->>'dueDate')::date,coalesce((item->>'amount')::numeric,0),nullif(item->>'paidDate','')::timestamptz,coalesce(item->>'status','UPCOMING'))
...
-- Keep the atomic sale primitive server-side and calculate profit from the locked product cost.
...
declare v_sale_id uuid; v_item jsonb; v_stock integer; v_cost numeric := 0;
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    select stock, purchase_price * (v_item->>'quantity')::numeric into v_stock, v_cost from products where id = (v_item->>'product_id')::uuid for update;
    ...
    v_cost := v_cost + (select purchase_price * (v_item->>'quantity')::numeric from products where id=(v_item->>'product_id')::uuid);
  end loop;
