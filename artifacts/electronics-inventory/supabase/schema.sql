...
-- Run this in Supabase SQL Editor before enabling cloud data.
...
create table if not exists public.emi_plans (
  ...
  start_date date not null, next_due_date date, end_date date not null,
  frequency text not null default 'MONTHLY', status text not null default 'ACTIVE'
);
create table if not exists public.emi_payments (
  ...
  installment_number integer not null, due_date date not null, amount numeric(12,2) not null,
  paid_date timestamptz, status text not null default 'UPCOMING'
);
...
-- Atomic checkout RPC: validates stock, inserts sale/items, and decrements stock in one transaction.
